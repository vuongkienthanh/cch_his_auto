...
def _filter_check_expand_sign(
    d: Driver,
    name: str,
    chuaky_fn: SIGN_AT_ROW_FN = _do_nothing,
    dangky_fn: SIGN_AT_ROW_FN = _do_nothing,
...
def sign_tobiabenhannhikhoa(d: Driver):
    def chuaky_fn(d: Driver, i: int):
        d.do_next_tab_do(
            f1=lambda d: _goto_row_and_click_edit(d, i), f2=editor_tobiabenhannhikhoa.bs
        )

    _filter_check_expand_sign(d, "Tờ bìa bệnh án Nhi khoa", chuaky_fn)


def sign_mucAbenhannhikhoa(d: Driver):
    def chuaky_fn(d: Driver, i: int):
        d.do_next_tab_do(
            f1=lambda d: _goto_row_and_click_edit(d, i), f2=editor_mucAbenhannhikhoa.bs
        )

    _filter_check_expand_sign(d, "Mục A - Bệnh án Nhi khoa", chuaky_fn)


def sign_mucBtongketbenhan(d: Driver):
    def chuaky_fn(d: Driver, i: int):
        d.do_next_tab_do(
            f1=lambda d: _goto_row_and_click_edit(d, i), f2=editor_mucBtongketbenhan.bs
        )
...
def sign_phieukhambenhvaovien(d: Driver):
    _filter_check_expand_sign(d, "Phiếu khám bệnh vào viện", _sign_current)


def sign_phieuchidinhxetnghiem(d: Driver):
    _filter_check_expand_sign(d, "Phiếu chỉ định xét nghiệm", _sign_current)


def sign_todieutri(d: Driver):
    def chuaky_fn(d: Driver, i: int):
        d.do_next_tab_do(
            f1=lambda d: _goto_row_and_click_edit(d, i), f2=editor_todieutri.bs
        )

    _filter_check_expand_sign(d, "Tờ điều trị", chuaky_fn)


def sign_phieuchidinhPTTT(d: Driver):
    _filter_check_expand_sign(d, "Phiếu chỉ định PTTT", _sign_current)


def sign_phieuCT(d: Driver, signature: str | None = None):
    def chuaky_fn(d: Driver, i: int):
        def f2_fn(d: Driver):
        ...
        d.do_next_tab_do(f1=lambda d: _goto_row_and_click_edit(d, i), f2=f2_fn)
...
def sign_phieuMRI(d: Driver, signature: str | None = None):
    def chuaky_fn(d: Driver, i: int):
        def f2_fn(d: Driver):
...
def sign_giaiphaubenh(d: Driver):
    def chuaky_fn(d: Driver, i: int):
        d.do_next_tab_do(
            f1=lambda d: _goto_row_and_click_edit(d, i), f2=editor_giaiphaubenh.bs
        )

    _filter_check_expand_sign(
        d, "Phiếu xét nghiệm giải phẫu bệnh sinh thiết", chuaky_fn
    )


def sign_phieusanglocdinhduong(d: Driver):
    _filter_check_expand_sign(
        d, "Phiếu sàng lọc dinh dưỡng - Bệnh nhi nội trú", _sign_current
    )


def sign_phieusoket15ngay(d: Driver):
    _filter_check_expand_sign(
        d, "Phiếu sơ kết 15 ngày điều trị", _sign_current_both, _sign_current2
    )


def sign_donthuoc(d: Driver):
    _filter_check_expand_sign(d, "Đơn thuốc", _sign_current)
