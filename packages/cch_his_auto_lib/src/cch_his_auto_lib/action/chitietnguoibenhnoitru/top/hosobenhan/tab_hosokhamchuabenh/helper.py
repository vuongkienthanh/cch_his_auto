...
def goto_row_then_tabdo(d: Driver, i: int, sign_fn: Callable):
    d.clicking(f"{RIGHT_PANEL} tr:nth-child({i})")
    tab0 = d.current_window_handle
    ...
    d.goto_newtab_do_smth_then_goback(tab0, sign_fn)
...
def is_row_expandable(d: Driver, idx: int) -> bool:
    ...
    for _ in range(5):
        time.sleep(1)
        try:
            ele = d.find(f"{RIGHT_PANEL} tr:nth-child({idx}) td:nth-child(1) button")
...
def filter_check_expand_sign(
    ...
    if filter(d, name) and (
        d.waiting(f"{RIGHT_PANEL} tr:nth-child(2) td:nth-child(3)").text.strip()
        != Status.HOANTHANH
    ):
